    ...
    let options = {
        ...
        fields : {
            eventNotes : {
                ...
                label : 'Notes',
                multiline : true,
                style : { flexDirection:'row', width: dw/2 , height : 60, padding: 4 }
            },
            eventDate : {
...
var TAG = "ContactNotesView.";
export default class ContactNotesView extends React.Component {
    constructor(props){
      super(props);
      ...
      this.state = { value : this.uistate.get().contactinfo.notes.defaults.toJS() }
    }
          ...
          <View style={{padding:15}}>
            <View style={styles.row}>
                ...
                <View style={{flex:0.15}} >
                    <Text>{data.eventDate ? moment(data.eventDate).format('D-M-YYYY') : ''}</Text>
                </View>

                <View style={{flex:0.8}} >
                    <Text>{data.eventNotes ? data.eventNotes.substr(0,600) : ''}</Text>
                </View>
    ...
    pressRow(rowId) {
      ...
      let rows = this.uistate.get().contactinfo.notes.data,
    ...
    addRow() {
        ...
        if (values) {
            ...
            if (!values.eventDate || !values.eventNotes ) {
                AlertIOS.alert(
                  'Errors',
                  'Please enter the date and notes for the event',
                  [
            ...
            let rows = this.uistate.get().contactinfo.notes.data.toJS()
            rows.push(values);
            this.uistate.get().contactinfo.set({refresh:true})
                                          .notes.set({data:rows}).now()
        } else {
